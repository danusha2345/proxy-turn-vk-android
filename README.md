...
На обоих VPS запущен и включён `wdtt-server.service`, бинарь находится в
`/opt/wdtt/wdtt-server`, конфигурация — в `/etc/wdtt/`, внутренний WireGuard-порт —
`56011/udp`. Инвентарные карточки хранятся локально в
`../Servera_vpn_andmo/servers/`.

Оба сервера обновлены 10 июля 2026 года из ветки `v1.2.4` и поддерживают режимы
`audio` (`PT=111`) и `video` (`PT=96`). Пароль подключения при обновлении не
менялся и в репозиторий или документацию не записывается.
