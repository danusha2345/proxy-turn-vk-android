...
- [Что нового в версии 1.2.4](#что-нового-в-версии-124)
...
- **RTP AEAD обфускация:** транспорт маскируется под RTP/WebRTC аудио (`PT=111`) или видео (`PT=96`), а полезная нагрузка шифруется AEAD-слоем поверх DTLS/WireGuard.
...
Проверенный 10 июля 2026 года рабочий endpoint: `185.245.34.224:56000/udp`.
На VPS запущен и включён `wdtt-server.service`, бинарь находится в
`/opt/wdtt/wdtt-server`, конфигурация — в `/etc/wdtt/`, внутренний WireGuard-порт —
`56011/udp`. Инвентарная карточка сервера хранится локально в
`../Servera_vpn_andmo/servers/vps-185-245-34-224.md`.

Сейчас на сервере работает совместимая версия `v1.2.0`. Режим `audio` остаётся
режимом по умолчанию и совместим с ней. Режим `video` (`PT=96`) следует включать
только после отдельного обновления серверного бинаря из этой версии проекта.
Пароль подключения в репозиторий и документацию не записывается.

## Что нового в версии 1.2.4

* **Два профиля маскировки:** совместимый `audio` (`PT=111`) и новый `video`
  (`PT=96`) с увеличенным случайным RTP-padding.
* **Структурированные события ядра:** Android и desktop-обёртки получают `READY`,
  `CONFIG` и `STATS` без разбора локализованных строк лога.
* **Протокольные тесты:** проверяются оба RTP payload type, шифрование round-trip,
  выдача WireGuard-конфига и отказ при несовпадении устройства.
